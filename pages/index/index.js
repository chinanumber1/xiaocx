...
Page((e = {
    ...
    onLoad: function(t) {
        ...
        qqmapsdk = new QQMapWX({
          key: 'T35BZ-AKWKI-THNGF-5TIX4-4BGBE-YZB2T'
        });  
    },
    onHide: function() {
        this.setData({
          ...
          success: function (res) {
            ...
            qqmapsdk.reverseGeocoder({
              ...
              success: function (res) {
                ...
                that.getListDetail(latitude, longitude, 0, 1);
              },
              fail: function (res) {
                console.log(res);
              },
  ...
  getListDetail: function (latitude, longitude, fenlei, sort){
        var that = this;
        wx.request({
          url: "https://o2goo.com/app/ewei_shopv2_api.php?i=4&r=shop.ajaxmerchuser233&comefrom=wxapp&openid=sns_wa_&mid=&merchid=&authkey=&timestamp=",
          data: {
            lat: latitude,
            lng: longitude,
            page: 1,
          ...
          success: function (ree) {
            var groupList = ree.data.result.list;
            var obj = groupList;
            ...
            for (var i = 0; i < obj.length; i++) {
              arr1[i] = ''
              for (var j = 0; j < obj[i].coupon.length; j++) {
                if (j == obj[i].coupon.length - 1) {
                  arr1[i] += obj[i].coupon[j].couponname
                } else {
                  arr1[i] += obj[i].coupon[j].couponname + ','
                }
              }
              // if (obj[i].distance == 100000) {
              //   distance.push('未显示商家距离')
              // } else {
              //   distance.push('距离您: ' + obj[i].distance + 'km')
              // }
              if (obj[i].sale == null) {

              } else {
                arr2[i] = ''
                arr2[i] += '满' + obj[i].sale.enoughmoney + '-' + obj[i].sale.enoughdeduct
                for (var j = 0; j < obj[i].sale.enoughs.length; j++) {
                  if (j == obj[i].sale.enoughs.length - 1) {
                    arr2[i] += ',' + '满' + obj[i].sale.enoughs[j].enough + '-' + obj[i].sale.enoughs[j].give
                  } else {
                    arr2[i] += ',' + '满' + obj[i].sale.enoughs[j].enough + '-' + obj[i].sale.enoughs[j].give
                  }
                }
            ...
            var groupCatename = ree.data.result.catename;
            that.setData({
              groupList: groupList,
              coupon: arr1,
              sale: arr2,
              groupCatename: groupCatename
            })
          },
          fail: function(err){
            console.log(err)
    ...
    fujin: function(){
      this.getListDetail(this.data.latitude, this.data.longitude, 0, 1);
    },
    allfenlei: function(){
      console.log(456)
      if (this.data.isshow[1] == 'none'){
        this.setData({
    ...
    queren: function(e){
      this.setData({
        ...
        fenleitext: e.currentTarget.dataset.text,
      })
      this.getListDetail(this.data.latitude, this.data.longitude, this.data.cateid, this.data.sorttype);
    },
    juli: function (e){
      this.setData({
        isshow: ['none', 'none', 'none'],
        sorttype: e.currentTarget.dataset.sorttype,
        zuijinjuli: e.currentTarget.dataset.text,
      })
      this.getListDetail(this.data.latitude, this.data.longitude, this.data.cateid, this.data.sorttype);
    },
    allsort: function(){
      if (this.data.isshow[2] == 'none') {
        this.setData({
          isshow: ['none', 'none', 'block']
        })
      } else {
        this.setData({
          isshow: ['none', 'none', 'none']
